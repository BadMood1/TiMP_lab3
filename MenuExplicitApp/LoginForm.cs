...
public class LoginForm : Form
{
    ...
    private void OnLoginClick(object? sender, EventArgs e)
    {
        ...
        try
        {
            var statuses = Authenticate(username, password);
            if (statuses == null)
            {
                MessageBox.Show(
    ...
    private static Dictionary<string, int>? Authenticate(string username, string password)
    {
        var dllPath = Path.Combine(AppContext.BaseDirectory, "AuthorizationLibrary.dll");
        if (!File.Exists(dllPath))
        {
            throw new FileNotFoundException($"Не найдена AuthorizationLibrary.dll: {dllPath}", dllPath);
        }
        ...
        var method = authType.GetMethod("Authenticate", new[] { typeof(string), typeof(string) })
            ?? throw new MissingMethodException(authType.FullName, "Authenticate");

        return method.Invoke(service, new object[] { username, password }) as Dictionary<string, int>;
    }
